      ...
      try {
        ...
        if (tableData) {
          for (var i = 0; i < tableData.length; i++) {
            ...
            // Find the current stage based on `Process`
            stage = Stages.filter(
              (subtask) => normalizeString(subtask.ID) === normalizeString(row.Process)
            );
        
            if (stage[0]) {
              const steps = stage[0].subtasks;
        
              // Ensure the current step from `tableData` exists in `steps`
              let step = steps.find((s) => s.ID === row.Step);
              if (!step) {
                // Create a new step if it doesn't exist
                const newStep = {
                  ID: row.Step,
                  Retailer: ret,
                  StartDate: new Date(row.Expected_Start),
                  EndDate: null,
                  Start_Date: new Date(row.Expected_Start),
                  End_Date: null,
                  SLA: row.SLA / 24,
                  Status: stage[0].Status === "Rejected" ? "Rejected" : "Not Started",
                  SLABreached: row.Is_SLA_breached === "Yes",
                  BaselineStartDate: new Date(row.Expected_Start),
                  BaselineEndDate: new Date(row.Expected_End),
                };
                steps.push(newStep);
              } else {
                // Update the existing step with table data
                step.BaselineStartDate = new Date(
                  row.Expected_Start ? row.Expected_Start : step.StartDate
                );
                step.BaselineEndDate = new Date(row.Expected_End);
                step.SLABreached = row.Is_SLA_breached === "Yes";
                step.SLA = row.SLA / 24;
        
                if (step.Status !== "Completed") {
                  step.Start_Date = new Date(step.StartDate);
                  step.End_Date = new Date(row.Expected_End);
                  step.StartDate = new Date(step.StartDate);
                  step.EndDate = new Date(row.Expected_End);
                } else {
                  step.Start_Date = new Date(step.StartDate);
                  step.End_Date = new Date(step.EndDate);
                  step.StartDate = new Date(step.StartDate);
                  step.EndDate = new Date(step.EndDate);
                }
              }
            } else {
              // If the stage does not exist, create it and the step
              const newStage = {
          ...
          // Additional processing of stages after mapping
          Stages = data[0].subtasks;
